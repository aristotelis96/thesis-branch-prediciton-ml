...
def get_pos_weight(traces, br_pc):
  num_taken = 0
  num_not_taken = 0
  num_taken_attr = 'num_taken_{}'.format(str(br_pc))
  num_not_taken_attr = 'num_not_taken_{}'.format(str(br_pc))
  for trace in traces:
...
def get_branch_indices(hdf5_file_ptr, br_pc):
  ...
  name of br_indices_{Branch PC in decimal}.
...
class BranchDataset(Dataset):
  ...
  def get_pos_weight(self):
    return get_pos_weight(self.trace_paths, self.br_pc)
